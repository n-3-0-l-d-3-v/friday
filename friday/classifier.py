...
from friday.config import GEMINI_API_KEY, GROQ_API_KEY
...
def detect_note_type(text, source):
    ...
    # `friday note "LC-76 minimum window substring"` still routes to the DSA
...
def build_classification_prompt(text, source, source_url, pre_type, dsa_pattern):
    ...
    return f"""
You are Friday, a precise AI classifier for a personal engineering knowledge OS.
...
def classify_with_groq(text, source, source_url, pre_type, dsa_pattern):
    ...
    try:
        ...
        from friday.ai import complete_json, last_error

        result = complete_json(prompt, max_tokens=700, temperature=0.1, prefer="groq")
        if result is None:
            print(f"  [Friday] Groq classification unavailable ({last_error()['groq']})")
            return None
        ...
        print("  [Friday] Groq request timed out")
        return None
    except httpx.RequestError as exception:
        print(f"  [Friday] Groq connection error: {exception}")
        return None
    except Exception as exception:
        print(f"  [Friday] Groq unexpected error: {exception}")
        return None
...
def classify_note(text, source, source_url):
    ...
    if GEMINI_API_KEY or GROQ_API_KEY:
        try:
            ...
            if parsed is not None:
                ...
                return normalized
            print(f"  [Friday] AI classifiers unavailable ({last_error()}) "
                  f"-- falling back to keywords")
        except Exception as exc:
            print(f"  [Friday] AI classification error: {exc}")
    ...
    if GROQ_API_KEY:
        ...
        if groq_result is not None:
            ...
            print("  [Friday] Classified via Groq")
            return normalized
        print("  [Friday] Groq failed -- using keyword classifier...")

    keyword_result = classify_with_keywords(text, source, source_url, pre_type, dsa_pattern)
    print("  [Friday] Classified via keywords (offline mode)")
...
---
*Captured: {timestamp} | Source: {source} | Friday*
'''
