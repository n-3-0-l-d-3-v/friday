...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Commit locally only — skip the GitHub push for instant capture (run 'friday push' later)",
)
@click.option(
    "--full",
    is_flag=True,
...
def note(text, source, url, force, no_push, full):
    ...
    if text.startswith(("https://youtube.com", "https://youtu.be", "https://www.youtube.com")):
        ...
        console.print(
            Panel(
                ...
                title="[bold]Friday — YouTube[/bold]",
                border_style="green",
                width=65,
            )
        )
        ...
        from friday.article_fetcher import process_article_url
    ...
    console.print(Panel(body, title="Friday", border_style="green"))
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Commit locally only — skip the GitHub push (run 'friday push' later)",
)
def process(force, no_push):
    ...
    if not files:
        ...
        return
    console.print(Panel(f"Found {len(files)} pending note(s)", title="Friday Processing", border_style="green"))
...
def push_cmd():
    ...
    from friday.git_sync import get_status, push_to_remote, stage_and_commit

    # Sweep up anything uncommitted first, then push everything.
    stage_and_commit("chore: friday batch sync")
    try:
        status = get_status()
...
def finalize(date_value):
    ...
    body = f"{summary}\n\nSaved to: {log_path}"
    console.print(Panel(body, title="Friday — Log Finalized", border_style="green"))
...
def weekly():
    ...
    console.print(Panel(body, title="Friday — Weekly Summary", border_style="blue"))
...
def schedule(rss, rss_time, curate, curate_time):
    ...
    if curate:
        from friday.scheduler import setup_curator_scheduler

        messages.append(setup_curator_scheduler(curate_time))
    body = "\n".join(messages)
    failed = any("Failed" in m for m in messages)
    console.print(Panel(body, title="Friday — Scheduler", border_style="red" if failed else "green"))
...
def status():
    ...
    console.print(Panel(body, title="Friday Status", border_style="blue"))
...
def sync_cmd():
    ...
    try:
        ...
        if sync_result.get("synced"):
            ...
            body = f"Error: {sync_result['push_error']}\nRun 'friday sync' again to retry pushing."
...
def cleanup_cmd():
    ...
    if sync_result.get("synced"):
        console.print(Panel("Cleanup complete and synced.", title="Friday Cleanup", border_style="green"))
    elif sync_result.get("committed") and sync_result.get("push_error"):
        console.print(Panel("Cleanup committed but push failed.", title="Friday Cleanup", border_style="yellow"))
    else:
        console.print(Panel("Cleanup complete.", title="Friday Cleanup", border_style="blue"))
...
def index_clean_cmd(fix_domains):
    ...
    from friday.index_store import dedupe_index
    ...
    if fix_domains:
        ...
        from friday.index_cleaner import fix_domains as run_fix_domains
    ...
    console.print(
        Panel(
            ...
            title="[bold]Friday — Index Cleanup[/bold]",
            border_style="green",
            width=50,
        )
    )
...
def article_cmd(url, personal_note):
    ...
    console.print(
        Panel(
            ...
            f"Saved  : {result['folder_path']}/{result['filename']}\n"
            f"GitHub : {'pushed' if sync_result.get('synced') else 'synced'}",
            title="[bold]Friday — Article[/bold]",
            border_style="green",
            width=65,
        )
    )
...
def discord_cmd():
    """Start the Friday Discord bot for mobile capture."""
    from friday.discord_bot import run_bot
    console.print(
        Panel(
            "Starting Friday Discord bot...\n\n"
            ...
            title="[bold]Friday — Discord Bot[/bold]",
            border_style="blue",
            width=55,
        )
    )
...
def link(domain):
    ...
    from friday.git_sync import sync
    from friday.linker import load_index, run_linker
    ...
    console.print(
        Panel(
            ...
            title="[bold]Friday — Cross-Linker[/bold]",
            border_style="cyan",
            width=50,
        )
    )
...
def curate_cmd(apply_changes, max_actions):
    ...
    from friday.curator import REVIEW, run_cycle
    ...
    if not result["entries"]:
        console.print(
            Panel("Nothing to maintain — the base is in good shape.",
                  title="Friday — Curator", border_style="green", width=64)
        )
        return
    ...
    console.print(Panel(body, title="[bold]Friday — Curator[/bold]",
...
def dedupe_cmd(threshold, apply_changes, no_archive):
    ...
    from friday.dedupe import dedupe
    ...
    if not result["cluster_count"]:
        console.print(
            Panel("No near-duplicate notes found.",
                  title="Friday — Dedupe", border_style="green", width=60)
        )
        return
    ...
    if apply_changes:
        ...
        body = (f"Merged   : {result['duplicate_count']} note(s)\n"
                ...
                f"Originals: {where}\n\n"
                f"[dim]Run 'friday link' to refresh wikilinks.[/dim]")
        style = "green"
    else:
    ...
    console.print(Panel(body, title="[bold]Friday — Dedupe[/bold]",
...
def daily_cmd(date_value):
    ...
    console.print(Panel(header, title="[bold]Friday — Daily[/bold]",
...
def listen_cmd(seconds, audio_file, ask_mode, dry_run):
    ...
    from friday.voice import has_microphone, record_fixed, record_until, transcribe_file
    ...
    if not text:
        console.print("[red]Could not transcribe (no speech detected, or "
                      "Groq unavailable — run 'friday doctor').[/red]")
        return
    ...
    if ask_mode:
        from friday.retrieval import ask

        result = ask(text)
        console.print(
            Panel(result["answer"], title="[bold]Friday — Answer[/bold]",
...
def wiki_cmd(topic, synth_all, min_size, dry_run):
    ...
    from friday.wiki import build_index, suggest_topics, synthesize_topic
    ...
    if not topic and not synth_all:
        ...
        console.print("[dim]Run: friday wiki \"<topic>\"[/dim]\n")
    ...
    if built and not dry_run:
        ...
        console.print(
            Panel(
                ...
                title="[bold]Friday — Wiki[/bold]", border_style="green", width=72,
            )
        )
...
def review_cmd(limit, domain, list_only):
    ...
    from friday.review import due_notes, record_review, review_stats
    ...
    if not due:
        console.print(
            Panel(f"Nothing due for review.\n\nTracked: {stats['tracked']} notes | "
                  ...
                  title="Friday — Review", border_style="green", width=60)
        )
        return
    ...
    console.print(
        Panel(f"{len(due)} note(s) due for review\n"
              ...
              title="[bold]Friday — Review[/bold]", border_style="cyan", width=60)
    )
...
def quiz_cmd(count, domain, note_type):
    ...
    from friday.review import generate_quiz
    ...
    console.print(
        Panel(f"Score: {score}/{len(questions)}",
              title="Friday — Quiz", border_style="cyan", width=40)
    )
...
def export_cmd(domain, tag, note_type, query, limit, title, output, no_toc):
    ...
    from friday.exporter import export
...
def open_cmd(query):
    ...
    from friday.retrieval import search_notes
...
def doctor_cmd(details, stale_days):
    ...
    if not ai["any"]:
        console.print("  [red]No AI provider is reachable — Friday is running on the "
    ...
    if not added:
        console.print(
            Panel(f"All {result['scanned']} notes on disk are indexed.",
                  title="Friday — Reindex", border_style="green", width=60)
        )
        return
...
def ask_cmd(question):
    ...
    console.print(
        Panel(
            ...
            title=f"[bold]Friday — {question[:50]}[/bold]",
            border_style="cyan",
            width=88,
        )
    )
...
def graph(search_term):
    ...
    from friday.linker import find_related_notes, load_index
...
def lc(number):
    ...
    from friday.leetcode_fetcher import fetch_problem
...
def rss_cmd(no_sync):
    ...
    from friday.rss_processor import process_feeds
...
def serve_cmd(host, port):
    ...
    console.print(
        Panel(
            f"[bold green]Friday API server starting...[/bold green]\n\n"
            f"Dashboard : [link]{dashboard_url}[/link]\n"
            f"Health    : http://{display_host}:{port}/health\n"
            f"Repo      : {REPO_PATH}\n\n"
            f"Open the dashboard, then drag the [bold]⚡ Save to Friday[/bold] button\n"
            f"to your bookmarks bar to capture pages from any browser.\n\n"
            f"[dim]Press Ctrl+C to stop.[/dim]",
            title="[bold]Friday — Serve[/bold]",
            border_style="green",
            width=68,
        )
    )
    from friday.api_server import run_server
