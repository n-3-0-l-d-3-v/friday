...
def fetch_problem(problem_number: int) -> Optional[dict]:
    try:
        ...
        if not slug:
            print("  [Friday] LeetCode: problem slug not found")
            return None
        ...
        if response.status_code != 200:
            print(f"  [Friday] LeetCode: fetch failed ({response.status_code})")
            return None
        ...
        print(f"  [Friday] LeetCode: fetch error ({exc})")
        return None
...
def enrich_note_with_leetcode(text: str, classification: dict) -> Optional[dict]:
    ...
    print(f"  [Friday] LeetCode: detected problem #{number}")
    data = fetch_problem(number)
    if not data:
        print("  [Friday] LeetCode: fetch failed, continuing without")
        return None

    title = data.get("title", "")
    difficulty = data.get("difficulty", "")
    print(f"  [Friday] LeetCode: fetched '{title}' ({difficulty})")
    return data
