"""
Retrieval layer for Friday — the recall half of the second brain.
...
of Friday. An inverted index can be added later if a repo ever gets huge.
"""

import re

from friday.config import GEMINI_API_KEY, GROQ_API_KEY, REPO_PATH
...
def _complete_with_gemini(prompt):
    from friday.ai import _try_gemini

    return _try_gemini(prompt, 1200, 0.2)


def _complete_with_groq(prompt):
    from friday.ai import _try_groq
...
def _ai_complete(prompt):
    ...
    from friday.ai import complete
...
def ask(question, k=8):
    ...
    if not candidates:
        return {
            "answer": "I couldn't find any notes related to that. Try `friday search` "
    ...
    prompt = f"""You are Friday, answering from a developer's personal notes.
