...
Design notes:
  - No external RSS library. Feeds are parsed with the stdlib xml.etree, which
    handles both RSS 2.0 (<item>) and Atom (<entry>) shapes. Keeps Friday light.
  ...
  - If GROQ_API_KEY is missing, a keyword heuristic is used as an offline-ish
    fallback so `friday rss` still does something useful.

Run manually:   friday rss
Run scheduled:  python -m friday.tasks rss   (wired via `friday schedule --rss`)
"""
...
from friday.config import REPO_PATH, INDEX_PATH, GROQ_API_KEY
...
def fetch_feed(feed):
    ...
    try:
        with httpx.Client(timeout=20.0, follow_redirects=True) as client:
            resp = client.get(url, headers={"User-Agent": "Friday-Knowledge-OS/1.0"})
...
def _filter_with_ai(candidates):
    ...
    prompt = f"""You are Friday, curating a developer's engineering knowledge base.
...
---
*Captured via Friday RSS on {timestamp[:10]} | Feed: {item.get('feed', '')}*
"""
...
def _existing_source_urls():
    ...
    keeps Friday from re-saving an item it already has a note for.
    """
    urls = set()
