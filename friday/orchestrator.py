...
from friday.classifier import classify_note
from friday.config import INDEX_PATH, REPO_PATH
from friday.daily_log import append_to_log, update_technologies
from friday.dsa_agent import analyze_dsa_note, build_dsa_note
from friday.formatter import format_note
from friday.index_store import upsert_note
from friday.leetcode_fetcher import enrich_note_with_leetcode
from friday.linker import run_linker, run_linker_for_new_notes, should_run_full_link
...
def _update_index(note_id, classification, timestamp, filename, source, enriched_data=None):
    ...
    # of adding a duplicate. See friday/index_store.py.
    upsert_note(entry)
...
def process_single_note(inbox_file_path: Path, force=False):
    from friday.capture import mark_processed
    ...
    try:
        ...
        if is_dsa and enriched_data:
            ...
            # `friday note --full` sets extra.lean = False on the captured payload.
...
def process_inbox_orchestrated(force=False, push=True):
    ...
    local and `friday push` (or the next default run) ships them together.
    """
    from friday.capture import list_pending, mark_processed, mark_failed
    from friday.git_sync import stage_and_commit, push_to_remote, build_commit_message
    ...
    if push and committed_any:
        ...
        print("  Committed locally (push skipped — run 'friday push' to sync)")
