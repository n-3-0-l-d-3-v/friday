"""
Friday local API server (Task 3.4).
...
Run it with:  friday serve   (or: python -m uvicorn friday.api_server:app)
...
captured from the browser is identical to `friday note`.
"""
...
from friday.config import INDEX_PATH, REPO_PATH

VERSION = "1.0"

app = FastAPI(title="Friday Knowledge OS", version=VERSION)
...
def api_analytics(days: int = 30):
    from friday.analytics import build_analytics
...
def api_graph(domain: str = "", max_nodes: int = 220):
    from friday.graph_view import build_graph
...
def capture_note_endpoint(body: NoteIn):
    ...
    from friday.orchestrator import process_inbox_orchestrated
...
def capture_article_endpoint(body: UrlIn):
    ...
    from friday.linker import run_linker_for_new_notes
...
def capture_youtube_endpoint(body: UrlIn):
    ...
    from friday.git_sync import sync
    from friday.linker import run_linker_for_new_notes
    from friday.youtube_agent import process_youtube_url
...
(function(){
  ...
  var old=document.getElementById('friday-capture-overlay');
  if(old){old.remove();}
  ...
  o.innerHTML=''
    ...
    +'<strong style="color:#00d4ff;letter-spacing:.3px">Friday Capture</strong>'
    ...
    +'<button id="jv-save" style="flex:1;background:#00d4ff;color:#001018;border:none;border-radius:8px;padding:10px;font-weight:600;cursor:pointer">Save to Friday</button>'
  ...
  document.getElementById('jv-save').onclick=function(){
    ...
    fetch(API+ep,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)})
      ...
      .catch(function(){msg('⚠ Cannot reach Friday. Is `friday serve` running?','#ff6b6b');});
  };
})();
"""
...
<title>Friday — Knowledge OS</title>
<style>
...
<div class="wrap">
  ...
  <div class="bm">
    ...
    <p style="color:var(--muted);font-size:13px;margin-top:0">
      ...
      YouTube video to save it straight into Friday — works in Zen, Firefox, Chrome, or mobile.
    </p>
    <p><a class="bm-drag" href="__BOOKMARKLET_HREF__" onclick="return false;">⚡ Save to Friday</a></p>
    <ol>
      <li>Make sure <code>friday serve</code> is running (this page proves it is).</li>
      <li>Show your browser's bookmarks bar (<b>Ctrl/Cmd + Shift + B</b>).</li>
      <li><b>Drag</b> the “⚡ Save to Friday” button up onto the bookmarks bar.</li>
  ...
  <footer>Friday Knowledge OS · dashboard served locally · nothing leaves your machine except your own GitHub pushes.</footer>
</div>
<script>
(function(){
