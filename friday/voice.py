"""
Voice capture and voice query (`friday listen`).

Speech-to-text runs on Groq's `whisper-large-v3-turbo`, which is on the same
free tier as the rest of Friday — no local model, no download, consistent with
the project's "cloud APIs only, keep the device light" constraint.

Two modes:
    friday listen           speak a note -> transcribed -> full capture pipeline
    friday listen --ask     speak a question -> answered from your own notes
...
def transcribe_file(path):
    ...
    from friday.ai import transcribe
