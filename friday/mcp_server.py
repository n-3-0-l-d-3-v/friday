"""
Friday MCP server — makes your knowledge base a native tool for any AI agent.

This is the "second interface" to Friday. Instead of typing CLI commands, you
...
CRITICAL: stdio transport uses stdout for the protocol itself. The Friday
...
server = MCPServer(
    name="friday",
    instructions=(
        "Friday is the user's personal engineering knowledge base: markdown notes "
...
def search_notes(query: str, limit: int = 10) -> str:
    with _quiet():
        from friday.retrieval import search_notes as _search
...
def ask_knowledge_base(question: str) -> str:
    with _quiet():
        from friday.retrieval import ask as _ask
...
def capture_note(text: str, source: str = "mcp", tags: str = "") -> str:
    ...
    with _quiet():
        from friday.capture import capture_note as _capture
        from friday.processor import process_inbox

        _capture(text, source=source or "mcp", source_url="")
        # push=False keeps the tool call fast; the user syncs with `friday push`.
    ...
    for r in reversed(result.get("results", [])):
        if r.get("success") and r.get("classification"):
            ...
            return (
                ...
                f"(type: {cls.get('type')}). Committed locally — run `friday push` to sync."
            )
...
def knowledge_stats() -> str:
    with _quiet():
        from friday.api_server import compute_stats
...
def notes_due_for_review(limit: int = 10, domain: str = "") -> str:
    with _quiet():
        from friday.review import due_notes, review_stats
...
def suggest_wiki_topics(min_size: int = 3) -> str:
    with _quiet():
        from friday.wiki import suggest_topics
...
def synthesize_topic(topic: str) -> str:
    ...
    with _quiet():
        from friday.wiki import build_index
        from friday.wiki import synthesize_topic as _synth
