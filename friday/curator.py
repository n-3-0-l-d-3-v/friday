"""
Autonomous curator (`friday curate`) — the knowledge base maintains itself.
...
def observe():
    ...
    from friday.health import check_health, health_score
    ...
    try:
        from friday.dedupe import find_duplicate_clusters
...
def act(action, state):
    ...
    try:
        ...
        if name == "relink":
            from friday.linker import run_linker

            out = run_linker(verbose=False)
            return {"done": True, "result": f"linked {out['linked']} note(s)"}

        if name == "synthesize":
            from friday.wiki import build_index, synthesize_topic
