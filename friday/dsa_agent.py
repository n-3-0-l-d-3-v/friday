...
from friday.config import GROQ_API_KEY
from friday.formatter import format_dsa
...
def analyze_dsa_note(text, classification, leetcode_data=None):
    if not GROQ_API_KEY:
        print("  [Friday] DSA Agent: no Groq key, skipping enrichment")
        return None
...
"""
    ...
    try:
        ...
        if parsed is None:
            print(f"  [Friday] DSA Agent: no usable response ({last_error()})")
            return None
        print(f"  [Friday] DSA Agent: enriched pattern={parsed.get('pattern')}")
        return parsed
    except Exception as exc:
        print(f"  [Friday] DSA Agent failed: {exc}")
        return None
...
---
*Captured: {timestamp} | Pattern: {pattern} | Friday DSA*
'''
